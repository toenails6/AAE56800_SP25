...
# Optimal functions. 
from mpc import mpc
from chaserTPBVP import chaserTPBVP
...
def generate_precise_figure8_waypoints(radius, altitude, num_points, x_offset, y_offset):
    """
    ...
    for theta in np.linspace(0, 2 * np.pi, num_points, endpoint=False):
        ...
        waypoints.append((x + x_offset, y + y_offset, altitude))

    waypoints.append((0.0 + x_offset, 0.0 + y_offset, altitude))  # return to center
    ...
    waypoints.append((0.0 + x_offset, 0.0 + y_offset, altitude))  # final return to center
...
if __name__ == "__main__":
    ...
    print('UAV0 MG',UAV0_HomePos.mg_heading)
    ...
    print('UAV1 MG',UAV1_HomePos.mg_heading)

    # === Generate Example Trajectory for UAV0 ===
    x_offset = UAV1_HomePos.x- UAV0_HomePos.x
    y_offset = UAV1_HomePos.y- UAV0_HomePos.y
    UAV0_wp = generate_precise_figure8_waypoints(5,3,100, 0, 0)
    UAV1_wp = generate_precise_figure8_waypoints(5,3,100, x_offset, y_offset)
    ...
    for i, (wp0, wp1) in enumerate(zip(UAV0_wp, UAV1_wp)):
        ...
        UAV1_GTT_history.append(np.array([FCN1.position.x, FCN1.position.y]))
        ...
        # UAV0_utm_y, UAV0_utm_x, UAV0_zone, UAV0_ut = utm.from_latlon(UAV0_Curr_GPS_x,UAV0_Curr_GPS_y)
        ...
        # print("UAV0 tempx and temp y", UAV0_utm_x, UAV0_utm_y)
        print('GPS0 Raw Loc',UAV0_raw_loc_x,UAV0_raw_loc_y)
        # print('UAV0 Home', UAV0_HomePos.x, UAV0_HomePos.y)
        ...
        # UAV1_utm_y, UAV1_utm_x, UAV1_zone, UAV1_ut = utm.from_latlon(UAV1_Curr_GPS_x,UAV1_Curr_GPS_y)
        UAV1_raw_loc_x = UAV1_utm_x - UAV0_HomePos.x # UAV1 local x with noise
        UAV1_raw_loc_y = UAV1_utm_y - UAV0_HomePos.y # UAV1 local y with noise
        # print("UAV1 tempx and temp y", UAV1_utm_x, UAV1_utm_y)
        print('GPS1 Raw Loc',UAV1_raw_loc_x,UAV1_raw_loc_y)
        # print('UAV1 Home', UAV1_HomePos.x, UAV1_HomePos.y)
        UAV1_GPS_history.append(np.array([UAV1_raw_loc_x, UAV1_raw_loc_y]))
        ...
        UAV1_EKF_history.append(UAV1_Xplus.copy())
        ...
        areaBnds = np.array([-40, 40, -30, 30])

        # +++ Insert MPC algorithm here +++
        mpcInput_x_est_evader = np.hstack([UAV0_Xplus[0:2].flatten(), UAV0_Xplus[3:5].flatten()])
        mpcInput_x_est_pursuer = np.hstack([UAV1_Xplus[0:2].flatten(), UAV1_Xplus[3:5].flatten()])
        mpcInput_Ts = timeStep
        # print(f"est shape{UAV0_Xplus.shape}")
        # print(f"1: {mpcInput_x_est_evader}")
        # print(f"2: {mpcInput_x_est_pursuer}")
        # print(f"3: {maxSpeed}")
        # print(f"4: {areaBnds}")
        # print(f"5: {mpcInput_Ts}")
        (x0 ,y0, z0) = mpc(mpcInput_x_est_evader, mpcInput_x_est_pursuer, maxSpeed, areaBnds, mpcInput_Ts) # replace wp0 with your algorithm, could use UAV0_Xplus and UAV1_Xplus as input
        # print(f"{x0}, {y0}")

        # +++ Insert TPBVP algorithm here +++
        chaserInput_x_c_0 = UAV1_Xplus[0:2].flatten()
        chaserInput_x_e_0 = np.hstack([UAV0_Xplus[0:2].flatten(), UAV0_Xplus[3:5].flatten()])
        (x1 ,y1, z1) = chaserTPBVP(chaserInput_x_c_0, chaserInput_x_e_0, maxSpeed, areaBnds, timeStep) # replace wp1 with your algorithm, could use UAV0_Xplus and UAV1_Xplus as input
        # print(f"{x1}, {y1}")
        ...
        # ===+++ Start of new waypoint Command +++===
        x1 = x1 - x_offset # DO NOT DELET THIS LINE!!! x Position Command Offset for Non Origin UAV (UAV1) 
        y1 = y1 - y_offset # DO NOT DELET THIS LINE!!! y Position Command Offset for Non Origin UAV (UAV1)

        FCN0.local_offboard_ctrl(x0,y0,z0,q0) # Send new waypoint command to UAV0
        FCN1.local_offboard_ctrl(x1,y1,z1,q1) # Send new waypoint command to UAV1
        # ===--- End of new waypoint Command ---===
...
EKF0 = np.array(UAV0_EKF_history).squeeze()       # shape (N, 9)
GPS0 = np.array(UAV0_GPS_history)                 # shape (N, 2)
GTT0 = np.array(UAV0_GTT_history)                 # shape (N, 3)
EKF1 = np.array(UAV1_EKF_history).squeeze()       # shape (N, 9)
GPS1 = np.array(UAV1_GPS_history)                 # shape (N, 2)
GTT1 = np.array(UAV1_GTT_history)                 # shape (N, 3)

# Extract x and y
EKF0_x, EKF0_y = EKF0[:, 0], EKF0[:, 1]
GPS0_x, GPS0_y = GPS0[:, 0], GPS0[:, 1]
GTT0_x, GTT0_y = GTT0[:, 0], GTT0[:, 1]
EKF1_x, EKF1_y = EKF1[:, 0], EKF1[:, 1]
GPS1_x, GPS1_y = GPS1[:, 0], GPS1[:, 1]
GTT1_x, GTT1_y = GTT1[:, 0], GTT1[:, 1]

# Plot
plt.figure(figsize=(8,6))
plt.plot(GTT0_x, GTT0_y, 'k--', label='UAV0 Ground Truth Track')
plt.plot(GPS0_x, GPS0_y, 'g:', label='UAV0 GPS Raw')
plt.plot(EKF0_x, EKF0_y, 'r-', label='UAV0 EKF Estimate')
plt.plot(GTT1_x, GTT1_y, 'k--', label='UAV1 Ground Truth Track')
plt.plot(GPS1_x, GPS1_y, 'g:', label='UAV1 GPS Raw')
plt.plot(EKF1_x, EKF1_y, 'b-', label='UAV1 EKF Estimate')
